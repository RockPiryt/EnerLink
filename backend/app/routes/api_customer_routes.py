...
def lookup_by_nip(nip):
    ...
    # Try GUS lookup if MF data is incomplete or missing, but only if NIP is valid
    if data and (not data.get("street") or not data.get("postcode") or not data.get("city")):
        ...
        return jsonify({**data, "source": "mf", "warning": "Incomplete address data in MF, GUS also did not return data."}), 200
    if data:
