...
def list_sales_customers():
    ...
    return jsonify([{
        "id": c.id,
        "name": getattr(c, "company", None) or getattr(c, "name", None),  # support both fields
    } for c in customers]), 200


# POST /sales/customers  { "customer_id": 1, "sales_rep_id": "SAL001" }
...
def assign_customer_to_sales_rep():
    ...
    # prevent duplicates (also protected by unique constraint)
    existing = Assignment.query.filter_by(customer_id=int(customer_id), sales_rep_id=str(sales_rep_id)).first()
    if existing:
        if not existing.active:
            existing.active = True
            db.session.commit()
        return jsonify({"message": "Customer assigned", "customer_id": int(customer_id), "sales_rep_id": str(sales_rep_id)}), 200
...
# GET /sales/analytics/contracts?year=2025
