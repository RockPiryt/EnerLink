...
def test_add_role_success(client):
    resp = client.post("/api/roles", json={"role_name": "Test Role"})
...
def test_get_role_not_found(client):
    resp = client.get("/api/roles/999999")
    assert resp.status_code == 404
    assert resp.get_json()["error"] == "Role not found"


def test_get_role_success(client):
    created = client.post("/api/roles", json={"role_name": "Role For Get"})
...
def test_update_role_not_found(client):
    resp = client.put("/api/roles/999999", json={"role_name": "X"})
    assert resp.status_code == 404
    assert resp.get_json()["error"] == "Role not found"


def test_update_role_success(client):
    created = client.post("/api/roles", json={"role_name": "Role To Update"})
    role_id = created.get_json()["role"]["id"]

    resp = client.put(f"/api/roles/{role_id}", json={"role_name": "Updated Role", "active": False})
    assert resp.status_code == 200
    data = resp.get_json()
...
def test_delete_role_not_found(client):
    resp = client.delete("/api/roles/999999")
    assert resp.status_code == 404
    assert resp.get_json()["error"] == "Role not found"


def test_delete_role_success(client):
    created = client.post("/api/roles", json={"role_name": "Role To Delete"})
    role_id = created.get_json()["role"]["id"]
