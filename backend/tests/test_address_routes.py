...
def test_get_countries_empty(client):
    resp = client.get("/address/countries")
...
def test_add_country_success(client):
    ...
    resp = client.post("/address/countries", json=payload)
...
def test_add_country_missing_fields(client):
    resp = client.post("/address/countries", json={"name": "Poland"})
    assert resp.status_code == 400
    assert resp.get_json()["error"] == "name and shortcut are required"

def test_patch_country_status_success(client):
    # najpierw dodaj kraj
    resp_create = client.post("/address/countries", json={"name": "Germany", "shortcut": "DE"})
    assert resp_create.status_code == 201
    country_id = resp_create.get_json()["country"]["id"]

    # zmień status
    resp = client.patch(f"/address/countries/{country_id}/status", json={"is_active": False})
    assert resp.status_code == 200
    data = resp.get_json()
...
def test_patch_country_status_not_found(client):
    resp = client.patch("/address/countries/999999/status", json={"is_active": False})
    assert resp.status_code == 404
    assert resp.get_json()["error"] == "Country not found"

def test_patch_country_status_missing_field(client):
    resp_create = client.post("/address/countries", json={"name": "France", "shortcut": "FR"})
    country_id = resp_create.get_json()["country"]["id"]

    resp = client.patch(f"/address/countries/{country_id}/status", json={})
    assert resp.status_code == 400
